...
export function ChatPanel({ isOpen, onClose, user, transactions }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 bg-cuanly-card border-l border-cuanly-border z-40 flex flex-col justify-between shadow-2xl shadow-slate-200/80 animate-slideLeft">
      {/* Header */}
      <div className="p-4 border-b border-cuanly-border flex items-center justify-between bg-cuanly-card">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <h3 className="text-xs font-black text-cuanly-textDark">Cuanly Chat</h3>
            <p className="text-[9px] text-cuanly-textMuted font-bold uppercase tracking-wider">Cuanly Intelligence</p>
          </div>
        </div>
        <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-slate-100 text-cuanly-textMuted hover:text-cuanly-textDark transition-colors">
          <X size={18} />
        </button>
      </div>

      {/* Mood Spending + Roast Mode bar */}
      <div className="px-4 py-2 border-b border-cuanly-border bg-slate-50 flex items-center justify-between text-xs">
        <div className="flex items-center space-x-1">
          <Smile size={14} className="text-cuanly-mint" />
          <span className="text-cuanly-textMuted">Mood Spending:</span>
          <span className="font-bold text-cuanly-textDark">{mood}</span>
        </div>
        ...
        <button
          ...
          className={`flex items-center space-x-1.5 px-3 py-1 rounded-full text-[10px] font-bold border transition-all duration-200 ${
            roastMode
              ...
              : 'border-slate-200 text-cuanly-textMuted hover:text-cuanly-textDark hover:bg-slate-100'
          }`}
        >
      ...
      <div className="px-4 py-2 border-b border-cuanly-border bg-cuanly-card flex space-x-2">
        <button
          ...
          className={`flex-1 py-1.5 rounded-lg text-[10px] font-bold border text-center transition-all duration-200 ${
            segment === 'b2c'
              ? 'bg-cuanly-violet/10 border-cuanly-violet/30 text-cuanly-violet'
              : 'border-transparent text-cuanly-textMuted hover:text-cuanly-textDark'
          }`}
        >
          Personal (B2C)
        </button>
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4">
          ...
          <div key={idx} className={`flex flex-col ${msg.isUser ? 'items-end' : 'items-start'}`}>
            <div className={`max-w-[85%] rounded-2xl p-4 text-xs leading-relaxed ${
              msg.isUser 
                ...
                : 'bg-slate-100 border border-slate-200/50 text-cuanly-textDark rounded-tl-none relative overflow-hidden group'
            }`}>
              ...
              {!msg.isUser && (
                <>
                  ...
                  {msg.directAnswer && (
                    <div className="text-lg font-black text-cuanly-violet tracking-tight mb-1">
                      {msg.directAnswer}
                    </div>
                  )}
                  {/* Part 2: Context Badge */}
                  {msg.contextBadge && (
                    <span className="inline-block px-2 py-0.5 rounded text-[9px] font-bold bg-white border border-slate-200 text-cuanly-mint mb-3 uppercase">
                      {msg.contextBadge}
                    </span>
                  )}
                </>
              ...
              <div className="flex flex-wrap gap-2 mt-2 max-w-[90%]">
                {msg.rekomendasi.map((rec, rIdx) => (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 hover:border-cuanly-violet/40 hover:bg-slate-100 text-[10px] font-bold text-cuanly-textMuted hover:text-cuanly-violet transition-all duration-200"
                  >
                    {rec}
                  </button>
                ))}
        ...
        {loading && (
          <div className="flex items-start">
            <div className="bg-slate-100 border border-slate-200/50 rounded-2xl rounded-tl-none p-3 flex space-x-1.5">
      ...
      <form onSubmit={handleSend} className="p-4 border-t border-cuanly-border bg-cuanly-card flex items-center space-x-2">
        <input
          ...
          className="flex-1 bg-slate-50 border border-cuanly-border rounded-xl px-4 py-3 text-xs focus:border-cuanly-violet focus:outline-none text-cuanly-textDark placeholder-slate-400"
        />
        <button
          type="submit"
